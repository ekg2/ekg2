...
COMMAND(cmd_metacontact)
{
	...
	if (match_arg(params[0], 'a', ("add"), 2)) {
		...
		if (metacontact_add(params[1])) {
			...
			query_emit(NULL, "metacontact_added", &tmp);
			xfree(tmp);
		}
		return 0;	
	}
	...
	if (match_arg(params[0], 'd', ("del"), 2)) {
		...
		if (metacontact_remove(params[1])) {
			...
			query_emit(NULL, "metacontact_removed", &tmp);
			xfree(tmp);
		}
		return 0;
	}
	...
	if (match_arg(params[0], 'i', ("add-item"), 2)) {
		...
		if (metacontact_add_item(m, params[2], params[3], atoi(params[4]), 0)) {
			...
			query_emit(NULL, "metacontact_item_added", &tmp1, &tmp2, &tmp3);
			xfree(tmp1);
			xfree(tmp2);
			xfree(tmp3);
		}
	...
	if (match_arg(params[0], 'r', ("del-item"), 2)) {
		...
		if (metacontact_remove_item(m, params[2], params[3], 0)) {
			...
			query_emit(NULL, "metacontact_item_removed", &tmp1, &tmp2, &tmp3);
			xfree(tmp1);
			xfree(tmp2);
			xfree(tmp3);
		}
...
{
	query_connect(NULL, "session_renamed", metacontact_session_renamed_handler, NULL);
	query_connect(NULL, "userlist_removed", metacontact_userlist_removed_handler, NULL);
}
