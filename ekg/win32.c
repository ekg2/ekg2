...
HANDLE win32_fork(thread_func_t addr, void *data) {
	...
	debug("NO_POSIX_SYSTEM: win32_fork() ADDR=0x%x data=0x%x; thread_id = %d result = %d\n", addr, data, thread_id, ret);
...
pid_t fork(void) {
	debug("NO_POSIX_SYSTEM: fork() ON NO POSIX SYSTEM DON'T USE FORK() REIMPLEMENT FUNCTION TO USE win32_fork()\n");
	errno = ERRNO_NOT;
	return -1;
}
...
int pipe(int filedes[2]) {
	...
	debug("NO_POSIX_SYSTEM: pipe() read=%d write=%d result=%d\n", pread, pwrite, res);
...
int fcntl(int fd, int cmd, long arg) {
	if (cmd == F_SETFL) {
		...
		debug("NO_POSIX_SYSTEM: fcntl() fd: %d F_SETFL  _NO_BLOCK: %d REST_args: %d\n", fd, no_block, arg);
		/* XXX */
	...
	debug("NO_POSIX_SYSTEM: fcntl() fd: %d cmd:%d arg:%d\n", fd, cmd, arg);
...
int ioctl(int fd, int request, void *flags) {
	...
	debug("NO_POSIX_SYSTEM: ioctl() fd: %d req: %d flags: 0x%x", fd, request, flags);
	errno = ERRNO_NOT;
	return -1;
}
