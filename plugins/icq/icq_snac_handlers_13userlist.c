...
SNAC_SUBHANDLER(icq_cmd_addssi_ack) {
	...
	if (!xstrcmp(cmd, "del")) {
		if ( (u = userlist_find(s, uid)) ) {
			...
			query_emit(NULL, "userlist_removed", &tmp, &uid);
			query_emit(NULL, "remove_notify", &s->uid, &uid);
		...
		if (!xstrcmp(cmd, "add")) {
			if ( (u = userlist_add(s, uid, nick)) ) {
				...
				query_emit(NULL, "userlist_added", &u->uid, &u->nickname, &quiet);
				query_emit(NULL, "add_notify", &s->uid, &u->uid);
			}
		} else {
			// modify
			if ( (u = userlist_find(s, uid)) ) {
				...
				if (nick) {
					...
					query_emit(NULL, "userlist_renamed", &u->nickname, &nick);
