...
Xajax ()
{
  this.DebugMessage = function (text)
  {
	...
	{
	  this.debugWindow =
	    window.open ('about:blank', 'xajax-debug',
			 'width=800,height=600,scrollbars=1,resizable,status');
	  this.debugWindow.document.
	    write
	    ('<html><head><title>Xajax debug output</title></head><body><h2>Xajax debug output</h2><div id="debugTag"></div></body></html>');
	}
      text = text.replace (/&/g, "&amp;")
	text = text.replace (/</g, "&lt;")
	text = text.replace (/>/g, "&gt;")
  ...
  this.addHandler = function (sElementId, sEvent, sFunctionName)
  {
	...
	eval ("this.$('" + sElementId + "').addEventListener('" + sEvent +
	      "'," + sFunctionName + ",false);");
      }
    else
      {
	sAltEvent = this.addOnPrefix (sEvent);
	eval ("this.$('" + sElementId + "').attachEvent('" + sAltEvent +
	      "'," + sFunctionName + ",false);");
      }
  }
  this.removeHandler = function (sElementId, sEvent, sFunctionName)
  {
    if (window.addEventListener)
      {
	sEvent = this.stripOnPrefix (sEvent);
	eval ("this.$('" + sElementId + "').removeEventListener('" + sEvent +
	      "'," + sFunctionName + ",false);");
      }
    else
      {
	sAltEvent = this.addOnPrefix (sEvent);
	eval ("this.$('" + sElementId + "').detachEvent('" + sAltEvent +
	      "'," + sFunctionName + ",false);");
      }
  }
  ...
  this.getInput = function (sType, sName, sId)
  {
    ...
    if (!window.addEventListener)
      {
	Obj =
	  document.createElement ('<input type="' + sType + '" id="' + sId +
				  '" name="' + sName + '">');
      }
    else
      {
		...
		if (formElements[i].type == 'select-multiple')
		  {
			...
			if (formElements[i].options[j].selected == true)
			  sXml +=
			    name + "=" +
			    encodeURIComponent (formElements[i].options[j].
						value) + "&";
		      }
		  }
		else
		  {
		    sXml +=
		      name + "=" + encodeURIComponent (formElements[i].value);
		  }
	      }
	  }
      }
		...
		for (var k = 0; k < child.childNodes.length; k++)
		  {
			...
			key = child.childNodes[k].firstChild.data;
		      ...
		      {
			value =
			  this._nodeToObject (child.childNodes[k].firstChild);
		      }
		  }
  ...
  this.call = function (sFunction, aArgs, sRequestType)
  {
      ...
      case xajaxDefinedGet:
	{
	  var uriGet =
	    uri.indexOf ("?") ==
	    -1 ? "?xajax=" + encodeURIComponent (sFunction) : "&xajax=" +
	    encodeURIComponent (sFunction);
	  if (aArgs)
	    {
      ...
      if (r.status == 200)
	{
	  ...
	  if (r.responseXML && r.responseXML.documentElement)
		...
		"Error: the XML response that was returned from the server is invalid.";
	      ...
	      trimmedResponseText =
		trimmedResponseText.replace (/\s + $ / g, "");
      ...
      else
	{
	  if (xajax.responseErrorsForAlert.containsValue (r.status))
	    {
	      var errorString =
		"Error: the server returned the following HTTP status: " +
		r.status;
	      errorString += "\nReceived:\n" + r.responseText;
	      alert (errorString);
	    }
    ...
    if (xajaxDebug)
      this.DebugMessage ("Calling " + sFunction + " uri=" + uri + " (post:" +
			 postData + ")");
    r.send (postData);
    if (xajaxStatusMessages == true)
	...
	if (xml.childNodes[i].nodeName == "cmd")
	  {
		...
		for (var j = 0; j < xml.childNodes[i].childNodes.length; j++)
		  {
		    if (xml.childNodes[i].childNodes[j].childNodes.length > 1
			&& xml.childNodes[i].childNodes[j].firstChild.
			nodeName == "#cdata-section")
		      {
			var internalData = "";
			for (var k = 0;
			     k <
			     xml.childNodes[i].childNodes[j].childNodes.
			     length; k++)
			  {
			    internalData +=
			      xml.childNodes[i].childNodes[j].childNodes[k].
			      nodeValue;
			  }
		      }
		    else
	      ...
	      else if (cmd == "pp")
		{
		  ...
		  eval ("objElement." + property + "=data+objElement." +
			property);
		}
	      else if (cmd == "rp")
		{
	      ...
	      else if (cmd == "ev")
		{
		  ...
		  eval ("this.$('" + id + "')." + property + "= function(){" +
			data + ";}");
		}
	      else if (cmd == "ah")
		{
	    ...
	    {
	      if (xajaxDebug)
		alert ("While trying to '" + cmdFullname +
		       "' (command number " + i +
		       "), the following error occured:\n" + e.name + ": " +
		       e.message + "\n" + (id
					   && !objElement ? "Object with id='"
					   + id + "' wasn't found.\n" : ""));
	    }
	    delete objElement;
	    delete cmd;
