...
char *jabber_openpgp(session_t *s, const char *fromto, enum jabber_opengpg_type_t way, char *message, char *key, char **error) {
	...
	switch (way) {
		case JABBER_OPENGPG_ENCRYPT:
			ret = new_guery_emit(NULL, "gpg_message_encrypt", &fromto, &message, &err);	break;
		case JABBER_OPENGPG_DECRYPT:
			ret = new_guery_emit(NULL, "gpg_message_decrypt", &s->uid, &message, &err);	break; 
		case JABBER_OPENGPG_SIGN:
			ret = new_guery_emit(NULL, "gpg_sign", &s->uid, &message, &err);			break;
		case JABBER_OPENGPG_VERIFY:
			ret = new_guery_emit(NULL, "gpg_verify", &fromto, &message, &key, &err);		break;	/* @ KEY retval key-id */
	}
