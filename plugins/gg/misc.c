...
static unsigned char *gg_iso_to_cp(unsigned char *buf) {
	...
	while (*buf) {
		if (*buf == (unsigned char)'°') *buf = '•';
		if (*buf == (unsigned char)'±') *buf = 'π';
		if (*buf == (unsigned char)'¶') *buf = 'å';
		if (*buf == (unsigned char)'∂') *buf = 'ú';
		if (*buf == (unsigned char)'¨') *buf = 'è';
		if (*buf == (unsigned char)'º') *buf = 'ü';
		buf++;
	}
	return tmp;
}
 ...
 * gg_blocked_remove()
 *
 * usuwa z listy blokowanych numerkÛw.
 *
 *  - uid
 */
 ...
 * gg_blocked_add()
 *
 * dopisuje do listy blokowanych numerkÛw.
 *
 *  - uid
 */
 ...
 *
 * zwraca tekst opisuj±cy powÛd b≥Ídu us≥ug po http.
 *
 *  - h - warto∂Ê gg_http->error lub 0, gdy funkcja zwrÛci≥a NULL.
 */
 ...
 * gg_userlist_send()
 *
 * wysy≥a do serwera listÍ uøytkownikÛw.
 */
