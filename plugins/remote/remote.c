...
static TIMER(remote_statusbar_timer) {
	...
	query_emit(NULL, "irc_topic", &irctopic, &irctopicby, &ircmode);
...
EXPORT int remote_plugin_init(int prio) {
	...
	query_connect(&remote_plugin, "ui_is_initialized", remote_ui_is_initialized, NULL);
	query_connect(&remote_plugin, "config_postinit", remote_postinit, NULL);

	query_connect(&remote_plugin, "ui_window_switch", remote_ui_window_switch, NULL);
	query_connect(&remote_plugin, "ui_window_kill", remote_ui_window_kill, NULL);
	query_connect(&remote_plugin, "ui_beep", remote_ui_beep, NULL);
	query_connect(&remote_plugin, "ui_window_print", remote_ui_window_print, NULL);
	query_connect(&remote_plugin, "ui_window_clear", remote_ui_window_clear, NULL);
	query_connect(&remote_plugin, "ui_window_new", remote_ui_window_new, NULL);
	query_connect(&remote_plugin, "ui_window_target_changed", remote_ui_window_target_changed, NULL);
	query_connect(&remote_plugin, "ui_window_act_changed", remote_ui_window_act_changed, NULL);
	query_connect(&remote_plugin, "variable_changed", remote_variable_changed, NULL);

	query_connect(&remote_plugin, "session_added", remote_session_added, NULL);

	/* SESSION_EVENT */
	query_connect(&remote_plugin, "protocol_connected", remote_protocol_connected, NULL);
	query_connect(&remote_plugin, "protocol_disconnected", remote_protocol_disconnected, NULL);

	query_connect(&remote_plugin, "session_changed", remote_session_changed, NULL);
	query_connect(&remote_plugin, "session_renamed", remote_session_renamed, NULL);

	query_connect(&remote_plugin, "userlist_changed", remote_userlist_changed, NULL);
	query_connect(&remote_plugin, "userlist_refresh", remote_userlist_refresh, NULL);
#if 0

	query_connect(&remote_plugin, "ui_window_target_changed", ncurses_ui_window_target_changed, NULL);
	query_connect(&remote_plugin, "ui_window_refresh", ncurses_ui_window_refresh, NULL);
	query_connect(&remote_plugin, "ui_window_update_lastlog", ncurses_ui_window_lastlog, NULL);
	query_connect(&remote_plugin, "ui_refresh", ncurses_ui_refresh, NULL);
	query_connect(&remote_plugin, "session_removed", ncurses_statusbar_query, NULL);
	query_connect(&remote_plugin, "binding_set", ncurses_binding_set_query, NULL);
	query_connect(&remote_plugin, "binding_command", ncurses_binding_adddelete_query, NULL);
	query_connect(&remote_plugin, "binding_default", ncurses_binding_default, NULL);
	query_connect(&remote_plugin, "conference_renamed", ncurses_conference_renamed, NULL);

	query_connect(&remote_plugin, "protocol_disconnecting", ncurses_session_disconnect_handler, NULL);
#endif

	/* podanie czegokolwiek jako data do remote_all_contacts_changed() powoduje wyzerowanie n->start */
	query_connect(&remote_plugin, "ui_refresh", remote_all_contacts_changed, (void *) 1);
	query_connect(&remote_plugin, "userlist_refresh", remote_all_contacts_changed, NULL /* ? */);

	query_connect(&remote_plugin, "session_changed", remote_all_contacts_changed, (void *) 1);
	query_connect(&remote_plugin, "session_event", remote_all_contacts_changed, NULL);

	query_connect(&remote_plugin, "metacontact_added", remote_all_contacts_changed, NULL);
	query_connect(&remote_plugin, "metacontact_removed", remote_all_contacts_changed, NULL);
	query_connect(&remote_plugin, "metacontact_item_added", remote_all_contacts_changed, NULL);
	query_connect(&remote_plugin, "metacontact_item_removed", remote_all_contacts_changed, NULL);

	query_connect(&remote_plugin, "userlist_added", remote_all_contacts_changed, NULL);
	query_connect(&remote_plugin, "userlist_removed", remote_all_contacts_changed, NULL);
	query_connect(&remote_plugin, "userlist_renamed", remote_all_contacts_changed, NULL);
